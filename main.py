...
"""
    ...
    def policy_page(filename: str):
        ...
        if text is None:
            return (
                '!!! warning "Impossible de charger cette page"\n'
                f"    Cette page est normalement récupérée automatiquement depuis [{filename} dans BentoBoxWorld/.github]({source_url}) lors de la construction du site, mais la récupération a échoué cette fois-ci. Consultez-la directement sur [GitHub]({source_url}) à la place.\n"
            )

        note = (
            '!!! note "Synchronisé depuis GitHub"\n'
            f"    Cette page est récupérée automatiquement depuis [{filename} dans BentoBoxWorld/.github]({source_url}) à chaque construction du site de documentation, elle correspond donc toujours à la politique officielle de l'organisation.\n\n"
        )
